...
def separate_records(filename):
    with open(filename, 'r') as fo:
        filename_three_plus_stars = "short_three_plus_stars.json"
        filename_two_minus_stars = "short_two_minus_stars.json"
    ...
    with open(filename, 'r') as fo:
        ...
        while True:
            ...
            if line is None or line is "":
                break
            data = json.loads(line)
            tokenized_words = set(list(bigrams(word_tokenize(data['text'].lower()))))
...
if __name__ == '__main__':
    ...
    filename = "yelp_short.json" # only useful for separation.
    # Write output to file in write mode.
    _fo = open("p2_summary2.txt", 'w')
    sys.stdout = _fo
